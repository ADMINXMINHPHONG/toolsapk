    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print("[!] Kivy khong co san - chay console mode")
            
            # Console mode - just show success
            print("\n" + "=" * 50)
            print("   APP DA KICH HOAT THANH CONG!")
            print("=" * 50)
            print(f"\nKey: {key_data['key']}")
            print(f"Het han: {format_time(key_data['expires'])}")
            print("\n[*] Nhan Enter de thoat...")
            input()
            
        except ImportError:
            print("[!] Khong co cryptography - thu giai ma base64...")
            # Fallback to base64 only
            try:
                decoded = base64.b64decode(encrypted_b64)
                decompressed = zlib.decompress(decoded)
                print(f"[OK] Decoded: {len(decompressed)} bytes")
                
                print("\n[OK] Da xac thuc thanh cong!")
